...
async function closeTicket(channel, closedBy, reason) {
  ...
  try {
    ...
    if (logsUrl) {
      components.push(
        new MessageActionRow().addComponents(
          new MessageButton().setLabel("Logs Ticket").setURL(logsUrl.short).setStyle("LINK")
        )
      );
    }
    ...
    const embed = new MessageEmbed().setAuthor("Ticket fechado").setColor(EMBED_COLORS.TICKET_CLOSE);
    if (reason) embed.addField("Razão", reason, false);
    embed
      .setDescription(`**Titulo:** ${ticketDetails.title}`)
      .addField("Aberto Por", ticketDetails.user ? ticketDetails.user.tag : "User left", true)
      .addField("Fechado Por", closedBy ? closedBy.tag : "User left", true);
...
async function closeAllTickets(guild, author) {
  ...
  channels.forEach(async (ch) => {
    const status = await closeTicket(ch, author, "Forçar o fechamento de todos os tickets abertos");
...
async function openTicket(guild, user, config) {
  ...
  try {
    ...
    const embed = new MessageEmbed()
      ...
      .setFooter("Você pode fechar seu ticket a qualquer momento clicando no botão abaixo");

    let buttonsRow = new MessageActionRow().addComponents(
      new MessageButton().setLabel("Fechar Ticket").setCustomId("TICKET_CLOSE").setEmoji("🔒").setStyle("PRIMARY")
    );
    ...
    const dmEmbed = new MessageEmbed()
      ...
      .setAuthor("Ticket Criado")
      .setThumbnail(guild.iconURL())
      .setDescription(`**Servidor:** ${guild.name}\n**Titulo:** ${config.title}`);

    const row = new MessageActionRow().addComponents(
      new MessageButton().setLabel("Ver Canal do Ticket").setURL(sent.url).setStyle("LINK")
    );
