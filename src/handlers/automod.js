...
async function performAutomod(message, settings) {
  ...
  if (mentions.members.size > automod.max_mentions) {
    embed.addField("Menções", `${mentions.members.size}/${automod.max_mentions}`, true);
    strikesTotal += mentions.members.size - automod.max_mentions;
  }

  // Maxrole mentions
  if (mentions.roles.size > automod.max_role_mentions) {
    embed.addField("Menções de Cargos", `${mentions.roles.size}/${automod.max_role_mentions}`, true);
  ...
  if (automod.max_lines > 0) {
    ...
    if (count > automod.max_lines) {
      embed.addField("Novas linhas", `${count}/${automod.max_lines}`, true);
      shouldDelete = true;
  ...
  if (automod.anti_links) {
    if (containsLink(content)) {
      embed.addField("Links encontrados", "✓", true);
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
    ...
    if (containsLink(content)) {
      ...
      if (message.client.antiScamCache.has(key)) {
        ...
        if (
          ...
          embed.addField("Detecção de AntiScam", "✓", true);
          shouldDelete = true;
          strikesTotal += 1;
        }
      } else {
  ...
  if (!automod.anti_links && automod.anti_invites) {
    if (containsDiscordInvite(content)) {
      embed.addField("Convites de Discord", "✓", true);
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (shouldDelete && message.deletable) {
    message
      .delete()
      .then(() => sendMessage(channel, "> Auto-Moderação! Mensagem deletada", 5))
      .catch(() => {});
  }
  ...
  if (strikesTotal > 0) {
    ...
    // send automod log
    embed
      .setAuthor("Auto Moderação")
      .setThumbnail(author.displayAvatarURL())
      .setColor(EMBED_COLORS.AUTOMOD)
      .setDescription(`**Canal:** ${channel.toString()}\n**Contente:**\n${content}`)
      .setFooter(`Por ${author.tag} | ${author.id}`, author.avatarURL());
    ...
    const strikeEmbed = new MessageEmbed()
      ...
      .setDescription(
        `Você recebeu ${strikesTotal} strikes!\n\n` +
          ...
          `**Total Strikes:** ${memberDb.strikes} fora de ${automod.strikes}`
      );
    ...
    if (memberDb.strikes >= automod.strikes) {
      await addModAction(message.guild.me, message.member, "Automod: Máximo de strikes recebidos", automod.action); // Add Moderation Action
