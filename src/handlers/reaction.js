...
async function handleFlagReaction(countryCode, message, user) {
  ...
  if (remaining > 0) {
    return sendMessage(message.channel, `${user} Você deve esperar ${timeformat(remaining)} antes de traduzir novamente!`, 5);
  }
  ...
  getCountryLanguages(countryCode, async (err, languages) => {
    ...
    const embed = new MessageEmbed()
      ...
      .setAuthor(`Tradução de ${src}`)
      .setDescription(desc)
      .setFooter(`Pedido por ${user.tag}`, user.displayAvatarURL());
