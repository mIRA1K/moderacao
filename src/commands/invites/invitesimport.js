...
module.exports = class InvitesImportCommand extends Command {
  constructor(client) {
    super(client, {
      name: "invitesimport",
      description: "Adicionar convites de servidor para usuários",
      category: "INVITE",
      ...
      slashCommand: {
        ...
        options: [
          {
            name: "user",
            description: "O usuário para importar convites para",
            type: "USER",
            required: false,
          },
        ],
...
async function importInvites({ guild }, user) {
  if (user && user.bot) return "Oops! Você não pode importar convites para bots";
  ...
  return `Feito! Convites anteriores adicionados a ${user ? user.tag : "todos os membros"}`;
}
