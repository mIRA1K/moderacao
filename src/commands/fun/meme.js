...
async function getRandomEmbed(choice) {
  ...
  if (!response.success) {
    return new MessageEmbed().setColor(EMBED_COLORS.ERROR).setDescription("Falha ao buscar meme. Tente novamente!");
  }

  const json = response.data;
  if (!Array.isArray(json) || json.length === 0) {
    return new MessageEmbed().setColor(EMBED_COLORS.ERROR).setDescription(`Nenhum meme encontrado correspondente ${choice}`);
  }
