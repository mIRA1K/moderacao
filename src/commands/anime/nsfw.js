...
module.exports = class NSFW extends Command {
  constructor(client) {
    super(client, {
      name: "nsfw",
      description: "Mostrar algum NSFW aleatório (+18)",
      enabled: true,
      category: "ANIME",
      cooldown: 10,
      validations: [
        {
          ...
          message: "Este comando só pode ser usado em canal NSFW",
        },
      ],
      command: {
        enabled: true,
...
const genNSFW = async (category, user) => {
  ...
  try {
    ...
    return new MessageEmbed().setImage(response.url).setColor("RANDOM").setFooter(`Pedido por ${user.tag}`);
  } catch (ex) {
    return new MessageEmbed()
      .setColor(EMBED_COLORS.ERROR)
      .setDescription("Falha ao buscar meme. Tente novamente!")
