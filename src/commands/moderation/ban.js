...
async function ban(issuer, target, reason) {
  ...
  if (typeof response === "boolean") return `${target.user.tag} está banido!`;
  if (response === "BOT_PERM") return `Eu não tenho permissão para banir ${target.user.tag}`;
  else if (response === "MEMBER_PERM") return `Você não tem permissão para banir ${target.user.tag}`;
