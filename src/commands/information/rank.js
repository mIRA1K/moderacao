...
module.exports = class Rank extends Command {
  constructor(client) {
    super(client, {
      name: "rank",
      description: "Mostra a classificação dos membros neste servidor",
      cooldown: 5,
...
async function getRank({ guild }, member) {
  ...
  if (!memberDb.xp) return `${user.tag} ainda não está classificado!`;
  ...
  if (!response.success) return "Falha ao gerar cartão de classificação";
